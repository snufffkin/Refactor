...
def page_admin(df: pd.DataFrame):
    ...
    with col1:
        if st.button("💾 Сохранить конфигурацию", type="primary"):
            ...
            # Предлагаем модифицировать файл core.py
            st.info("""
            Чтобы применить новую конфигурацию, вам нужно обновить значения в файле core.py.
            
            Вы можете скопировать значения из файла конфигурации или использовать 
            функцию загрузки конфигурации при запуске приложения.
            """)
    ...
    with st.expander("📖 Подробное описание формулы риска", expanded=False):
        ...
        3. **Учет максимального риска** - комбинируется взвешенное среднее и максимальный риск
        4. **Минимальный порог** - устанавливается минимальный порог риска для критических метрик
        5. **Корректировка на статистическую значимость** - учитывается количество попыток
        """)
        ...
        где α - коэффициент баланса между взвешенным средним и максимальным риском (обычно 0.7)
        """)
        ...
        **Итоговый риск** = {raw_risk:.3f} × {confidence:.2f} + {config["stats"]["neutral_risk_value"]:.2f} × (1 - {confidence:.2f}) = **{final_risk:.3f}**
        ...
        # Добавляем примеры краевых случаев
        st.markdown("## Примеры крайних случаев")
        
        ex_col1, ex_col2 = st.columns(2)
        
        with ex_col1:
            st.markdown("### Пример 1: Высокий риск одной метрики")
            st.markdown("""
            Предположим, что у карточки все метрики в норме, но очень много жалоб:
            * Дискриминативность: 0.42 (риск 0.05)
            * Успешность: 0.82 (риск 0.10)
            * Успешность с 1-й попытки: 0.75 (риск 0.10)
            * Количество жалоб: 60 (риск 0.80)
            * Доля пытавшихся: 0.92 (риск 0.08)
            * Всего попыток: 300
            
            В этом случае:
            * Взвешенное среднее: ≈0.32
            * Максимальный риск: 0.80
            * Комбинированный риск: ≈0.46
            * Минимальный порог: 0.60
            * Итоговый риск: 0.60
            
            Карточка получает высокий риск из-за большого количества жалоб, 
            несмотря на хорошие показатели по другим метрикам.
            """)
        
        with ex_col2:
            st.markdown("### Пример 2: Баланс метрик")
            st.markdown("""
            Предположим карточку с умеренными значениями всех метрик:
            * Дискриминативность: 0.25 (риск 0.37)
            * Успешность: 0.65 (риск 0.35)
            * Успешность с 1-й попытки: 0.55 (риск 0.38)
            * Количество жалоб: 7 (риск 0.35)
            * Доля пытавшихся: 0.75 (риск 0.40)
            * Всего попыток: 250
            
            В этом случае:
            * Взвешенное среднее: ≈0.37
            * Максимальный риск: 0.40
            * Комбинированный риск: ≈0.38
            * Минимальный порог: 0.00
            * Итоговый риск: 0.38
            
            Карточка получает умеренный риск, так как все метрики 
            находятся в диапазоне среднего риска.
            """)
        
        # Добавляем раздел с советами по интерпретации и использованию
        st.markdown("## Рекомендации по интерпретации и использованию")
        
        st.markdown("""
        ### Общие рекомендации
        
        1. **Фокусируйтесь на метриках с высоким риском** - они вносят наибольший вклад в общий риск
        2. **Учитывайте статистическую значимость** - данные с малым количеством попыток менее надежны
        3. **Настраивайте веса в зависимости от приоритетов** - выделяйте более важные для вас метрики
        4. **Регулярно пересматривайте пороговые значения** - они могут меняться в зависимости от типов заданий
        
        ### Метрики и их интерпретация
        
        * **Дискриминативность** - показывает, насколько хорошо задание различает знающих и незнающих студентов
        * **Успешность** - общая доля успешных решений задания
        * **Успешность с первой попытки** - доля студентов, решивших задание с первой попытки
        * **Количество жалоб** - абсолютное число жалоб на задание
        * **Доля пытавшихся** - какая часть студентов попыталась решить задание
        
        ### Пример анализа
        
        1. Если у карточки высокий риск, определите, какая метрика вносит наибольший вклад
        2. Если это жалобы - проверьте формулировку задания и исправьте возможные неточности
        3. Если это низкая успешность - подумайте об упрощении задания или добавлении подсказок
        4. Если это низкая дискриминативность - возможно, задание слишком простое или слишком сложное для всех
        """)
